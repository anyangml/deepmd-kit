...
@DescriptorBlock.register("hydra_repformer")
class DescrptBlockHydraRepformers(DescriptorBlock):
    def __init__(
        self,
        rcut,
        rcut_smth,
        ...
        old_impl: bool = False,
        hydra_layers: List[int] = [4,6,8,12]
    ):
        r"""
        ...
        self.hydra_layers = hydra_layers
    ...
    def forward(
        ...
        g1 = torch.stack(g1s,dim=1)
        g2 = torch.stack(g2s, dim=1)
        h2 = torch.stack(h2s, dim=1)
        rot_mat = torch.stack(rot_mats, dim=1)

        return g1, g2, h2, rot_mat.view(-1, len(self.hydra_layers), nloc, self.dim_emb, 3), sw
