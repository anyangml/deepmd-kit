...
@BaseAtomicModel.register("hydra")
class EnergyHydraAtomicModel(BaseAtomicModel):
    """Model give atomic prediction of some physical property.

    Parameters
    ----------
    descriptor
            Descriptor, this descriptor should have one extra dimension of nlayers compared to regular models.
    fitting_nets
            Fitting nets, this should be a list of fitting nets, one for each layer. 
            This fitting nets configurations need to be the same for simplicity.
    type_map
    ...
    def change_type_map(
        ...
        for fitting_net in self.fitting_nets:
            fitting_net.change_type_map(type_map=type_map)
        ...
        dd.update(
            {
                ...
                "fitting_nets": [fn.serialize() for fn in self.fitting_nets]
            }
        )
        return dd

    @classmethod
    def deserialize(cls, data) -> "EnergyHydraAtomicModel":
        data = copy.deepcopy(data)
        check_version_compatibility(data.pop("@version", 1), 2, 1)
        data.pop("@class", None)
        data.pop("type", None)
        descriptor_obj = BaseDescriptor.deserialize(data.pop("descriptor"))
        fitting_nets = [BaseFitting.deserialize(fn) for fn in data.pop("fitting_nets")]
        data["descriptor"] = descriptor_obj
        data["fitting_nets"] = fitting_nets
    ...
    def forward_atomic(
        ...
        # these values now have an extra dimension of hydra_layer
        ...
        assert len(descriptor.shape) == 4, "descriptor missing 1 dimension"
