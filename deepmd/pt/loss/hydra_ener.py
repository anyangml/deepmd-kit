...
class HydraEnergyStdLoss(TaskLoss):
    def __init__(
        self,
    ...
    def forward(self, input_dict, model, label, natoms, learning_rate, mae=False):
        ...
        if self.has_e and "energy" in model_pred and "energy" in label:
            ...
            energy_label = label["energy"].unsqueeze(1).expand(-1,len(self.hydra_layers) ,-1)
            if self.enable_atom_ener_coeff and "atom_energy" in model_pred:
                ...
                energy_pred = torch.sum(atom_ener_coeff * atom_ener_pred, dim=2)
        ...
        if self.has_v and "virial" in model_pred and "virial" in label:
            ...
            diff_v = label["virial"].unsqueeze(1).expand(-1,len(self.hydra_layers) ,-1) - model_pred["virial"].reshape(-1, 9)
