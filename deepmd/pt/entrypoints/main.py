...
def grad_probe(FLAGS) -> None:
    ...
    for task_key in trainer.model_keys:
        ...
        for _ in range(FLAGS.nbatches):
            ...
            if FLAGS.mae:
                # Manual normalized MAE calculation
                ...
                if "energy" in model_pred and "energy" in label_dict:
                    natoms = input_dict["atype"].shape[-1]
                    e_pred = model_pred["energy"]
                    e_label = label_dict["energy"]
                    # Normalized Energy MAE: 1/N * mean(|E_pred - E_label|)
                    loss += torch.mean(torch.abs(e_pred - e_label)) / natoms
                if "force" in model_pred and "force" in label_dict:
                    f_pred = model_pred["force"]
